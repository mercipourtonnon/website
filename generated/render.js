...
const pageConfig = JSON.parse(
  fs.readFileSync(path.join(__dirname, "page.json"), "utf-8"),
);
...
function renderPage() {
  ...
  const headContent = renderToString(
    <Components.Head meta={meta} theme={theme} />,
  );
  ...
  const bodyContent = renderToString(
    <>
      ...
      {sections.map((section, index) => (
        <Components.Render key={index} element={section} blocs={blocs} />
      ))}
    </>,
  );

  // Render Scripts component
  const scriptsContent = renderToString(
    <Components.Scripts scripts={scripts} />,
  );
